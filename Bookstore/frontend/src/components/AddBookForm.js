...
const AddBookForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Find the IDs for the selected author and genre
      const selectedAuthor = authors.find((author) => author.name === authorName);
      const selectedGenre = genres.find((genre) => genre.genre_name === genreName);

      if (!selectedAuthor || !selectedGenre) {
        toast.error('Invalid author or genre selected.'); // Show error message
        return;
      }

      const response = await axios.post('http://localhost:5000/api/books', {
        title,
        author_id: selectedAuthor.author_id,  // Send author ID
        genre_id: selectedGenre.genre_id,      // Send genre ID
