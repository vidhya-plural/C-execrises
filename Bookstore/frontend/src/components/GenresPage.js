...
const GenresPage = () => {
  ...
  const handleSaveEdit = async (genreId) => {
    try {
      const response = await axios.put(
        ...
        { genre_id: genreId, genre_name: newGenreName },
      ...
      if (response.status === 200) {
        ...
        setGenres(genres.map(genre => {
          if (genre.genre_id === genreId) {
            return { ...genre, genre_name: response.data.genre_name };
          }
          return genre;
        }));
      ...
      console.error('Error updating genre:', error.response || error.message);
      toast.error(`Error updating genre: ${error.response?.data || error.message}`);
    }
  };
  ...
  const handleAddGenre = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/genres', { genre_name: newGenreName });
