...
const UserLoginForm = () => {
  ...
  const handleLoginSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/UserLogin', { username, password });
  ...
  const handleRegisterSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/members', { name, email, username, password });
      console.log(response.data); // For testing purposes, can be replaced with redirection logic
